...
if(isset($_POST['ab']))
{	
	$query1 = "UPDATE test1 SET T1DBMS='$_POST[num1]' WHERE USN='$_POST[hidden]'";
	$query_run1 = mysqli_query($connect,$query1);
	$query2 = "UPDATE test2 SET T2DBMS='$_POST[num2]' WHERE USN='$_POST[hidden]'";
	$query_run2 = mysqli_query($connect,$query2);
	$query3 = "UPDATE test3 SET T3DBMS='$_POST[num3]' WHERE USN='$_POST[hidden]'";
	$query_run3 = mysqli_query($connect,$query3);
	
	
	$query4 = "UPDATE noofclassesattended SET DBMS='$_POST[num5]'WHERE USN='$_POST[hidden]'";
	$resZ=mysqli_query($connect,$query4);
	$st=$_POST['num5'];
	$queryY = "UPDATE attendance_teacher SET DBMS=(SELECT ATTENDANCE(\"$st\") AS DBMS) WHERE USN='$_POST[hidden]'";
	$resY=mysqli_query($connect,$queryY);
	$qry = "Select * from attendance_teacher where USN='$_POST[hidden]'";
	$ress=mysqli_query($connect,$qry);

};
$query = "SELECT A.USN,B.T1DBMS,C.T2DBMS,D.T3DBMS
...
{
	echo "<form action=dbms.php method=post>";
	echo "<tr>";
	echo  "<td>".$row['USN']."</td>";
	
	echo "<td><input type=number name=num1 value=".$row['T1DBMS']."></td>";
	
	echo "<td><input type=number name=num2 value=".$row['T2DBMS']."></td>";

	echo "<td><input type=number name=num3 value=".$row['T3DBMS']."></td>";
	
	?><?php
	
$var=$row['USN'];

$query100 = "SELECT A10.USN,A10.DBMS
...
$row100=mysqli_fetch_array($res100);
	echo "<td><input type=number name=num5 value=".$row100['DBMS']."></td>";
	
	$queryA = "SELECT A1.USN,A1.DBMS
...
$rowA=mysqli_fetch_array($resA);
	  ...
	  echo "<td><input name=num4 value=".$rowA['DBMS']."></td>";
